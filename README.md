...
画面のデザインを変更せず、学習セッション（条文閲覧を含む）が開始された時点で1件のイベントをSharePointへ追加します。同じセッション中の次の問題・次の条文は追加記録しません。

既存の全体カウンター用リスト `stiaccesscounter` は残し、利用者別イベント用に `stiuseraccess` リストを作成します。`stiuseraccess` には次の列を用意してください。

- `Title`: 1行テキスト。イベント識別子が入ります。
